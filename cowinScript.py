...
API_KEY = "o.sxZgU5wdqyxvaDkdHXOe51Apvma3jD4E"
...
query = "/api/v2/appointment/sessions/public/findByDistrict?district_id=506&date="+str(d1)
conn.request("GET", query, payload, headers)
res = conn.getresponse()
data = res.read()
dataUTF = data.decode("utf-8")
...
for x in dataDICT:
    if(isinstance(dataDICT[x], list)):
        count += len(dataDICT[x])
now = (datetime.now()).strftime("%H:%M:%S")
ininfo2 = f"Vaccination Slot Info JPR2 - {d1} {now}"
info2 = ""
# printing centres information
for i in range(0,count):
    dose1 = dataDICT["sessions"][i]['available_capacity_dose1']
...
API_KEY = "o.sxZgU5wdqyxvaDkdHXOe51Apvma3jD4E"
pb = PushBullet(API_KEY)
push = pb.push_note(ininfo2, info2)